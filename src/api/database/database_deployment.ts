...
import { admin_user, tester_users } from './conf';
...
const test_super_user_exist = async (): Promise<void> => {
  ...
  if (!(await user_exist(admin.user_login))) {
    ...
    const admin_id: number = await new Promise<number>((resolve) => {
      create_user_db(admin, (error: MysqlError | null, results: any) => {
    ...
    if (admin_id !== -1) {
      ...
      const admin_role_id: number = await new Promise<number>((resolve) => {
        get_role_by_role_name_db('admin', (error: MysqlError | null, results: any) => {
          console.log(results);
          if (error) console.log(`Failed to get role is for 'admin' role!\n${error.message}`);
...
const test_tester_users_exist = async (): Promise<void> => {
  for await (const tester_user of tester_users) {
    console.log(`Checking if tester user '${tester_user.user_login}' exists . . .`);
    if (!(await user_exist(tester_user.user_login))) {
      console.log(`Tester user '${tester_user.user_login}' doesn't exist, creating . . .`);
      let tester_user_db: User = { ...tester_user };
      tester_user_db.password = hashSync(tester_user.password, salt);
      const tester_user_id: number = await new Promise<number>((resolve) => {
        create_user_db(tester_user_db, (error: MysqlError | null, results: any) => {
          if (error) console.log(`Failed to create tester user '${tester_user.user_login}'!\n${error.message}`);
          if (results) {
            if (typeof results.insertId === 'number') {
              console.log(`Successfully created tester user '${tester_user.user_login}'! id is '${results.insertId}'`);
              resolve(results.insertId);
            } else resolve(-1);
          }
        });
      ...
      if (tester_user_id !== -1) {
        await new Promise<void>((resolve) => {
          set_protected_users_db([tester_user_id], (error: MysqlError | null, results: any) => {
            if (error) console.log(`Failed to set protection for tester user '${tester_user.user_login}'!\n${error.message}`);
            if (results) {
              console.log(`Successfully set protection for tester user '${tester_user.user_login}'!`);
            }
            resolve();
          });
        });
        const tester_user_role_id: number = await new Promise<number>((resolve) => {
          get_role_by_role_name_db('QA', (error: MysqlError | null, results: any) => {
            // console.log(results);
            if (error) console.log(`Failed to get role id for 'QA' role!\n${error.message}`);
            if (typeof results[0].role_id === 'number') resolve(results[0].role_id);
            else resolve(-1);
          });
        });
        if (tester_user_role_id !== -1)
          await new Promise<void>((resolve) => {
            create_account_db(
              {
                user_id: tester_user_id,
                role_id: tester_user_role_id,
                account_status: 'active',
                termination_date: null,
              },
              (error: MysqlError | null, results: any) => {
                if (error) console.log(`Failed to create tester user '${tester_user.user_login}' account!\n${error.message}`);
                if (typeof results.insertId === 'number')
                  console.log(`Successfully created tester user '${tester_user.user_login}' account! account id is '${results.insertId}'`);
                else console.log(`Failed to created tester user '${tester_user.user_login}' account!`);
                resolve();
              }
            );
          });
      }
    } else console.log(`Tester user '${tester_user.user_login}' exist!`);
  }
};
